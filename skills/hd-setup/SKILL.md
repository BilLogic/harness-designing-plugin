...
- **Step 4 — Layer 1 (Context).** Semantic memory. Create under `docs/context/`; scaffold writes pointer files with 3–5 line extracted summaries; review applies bloat-detection. → [layer-1-context.md](references/layer-1-context.md). **Voice sub-step (v6 / DS bootstrap):** when `signals.voice_docs_found` is empty (or user opts to extend), run the 3-batched-question Voice / Use case / Forbidden moves authoring procedure → [voice-authoring.md](references/voice-authoring.md). When `signals.design_system_scenario != "starter"` AND user wants depth, suggest `/hd:design-system establish` for the design-system sub-flow.
