...
- **Migrate-on-adopt** (3z, schema v1): of the 17 starters, 6 already ship in YAML schema v1 (`skill-quality`, `ux-writing`, `heuristic-evaluation`, `plan-quality`, `lesson-quality`, `agent-spec-quality`, `rubric-template`). The other 11 are legacy prose-only by design — they ship as reference material until adopted. When a user adopts a legacy starter into `docs/rubrics/`, walk them through adding `version: 1` + `source:` + a `sections:` map per [`../../hd-review/references/rubric-yaml-schema.md`](../../hd-review/references/rubric-yaml-schema.md) so `rubric-applier` can parse criteria deterministically. Do NOT eagerly migrate unadopted starters — the prose tables are intentional reference material until a team commits.
