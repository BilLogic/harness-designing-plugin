# Rule adoption criteria

**Purpose:** rules for when a Layer 5 lesson is promoted to a Layer 1 / AGENTS.md rule. Loaded by [`../SKILL.md` § Propose mode](../SKILL.md) before drafting a rule proposal.

## The three criteria (all must hold)

A lesson qualifies for rule adoption when:
...
3. **Team agreement.** The team (or sole maintainer) agrees this should be a rule, not just a suggestion. Agreement comes via RFC, PR conversation, sync meeting, or explicit approval in the `apply-rule` flow.
...
- For each tag in the proposed rule's topic set, count lessons tagged with it
- Union-count across the topic's tags
- Filter out duplicates (same slug, different file)
- Threshold: ≥3 distinct lessons

The `propose-rule` workflow automates this. If count < 3, workflow aborts with:

> Found N matching lessons. Need ≥3 for rule adoption. Add more lessons as the pattern recurs, or document why this warrants an exception.

## Clean-imperative test

Before proposing a rule, check: can the rule be stated in ≤1 sentence as a clean imperative?

**Good imperatives** (rule-ready):
...
Agreement is captured in the `apply-rule` workflow:

1. Proposer runs `propose-rule` → emits plan + hash
2. Reviewer(s) read the plan
3. Agreement signaled by running `apply-rule --plan-hash <sha>` with the exact hash
4. `approver` field in `changelog.md` entry records who approved
...
Explicit cases where rule adoption is NOT appropriate:
...
Rules tied to specific tech, vendor, or external state that may change. Example: "Use v1 API before August 2025." Put such conditional statements in Layer 1 context or a collapsible `<details>` section — they're not rule-grade rules.
...
If proposing the rule would trigger team debate, it's not ready. Rule adoption is formalization of a consensus; if there's no consensus, rule adoption is premature. Let the debate play out first — capture it in lessons as "team discussed X; no consensus; revisit Q3" — then re-propose when consensus emerges.

## Rule revocation (reversing a rule)

If a rule turns out wrong, don't delete it — revoke a rule:
...
2. Propose rule revocation: removes the rule from AGENTS.md, appends a "revoked" meta-entry to `changelog.md`, links both lessons (original + revocation)
3. Plan-hash verifies; apply atomically

The original rule's AGENTS.md line is removed (we edit Tier 1); the original LESSON is preserved (history is sacred); the changelog.md gets a new entry showing the revocation.

**Rule revocation is rare.** If you find yourself revoking frequently, rule adoptions are premature. Tighten the criteria.

## See also

- [lesson-patterns.md](lesson-patterns.md) — authoring lessons well sets up good rule adoptions
