...
**DS sub-validation (v6 / DS bootstrap)** — when `signals.design_system_scenario != "starter"` OR `docs/context/design-system/index-manifest.json` exists, the L1 audit ALSO delegates to `/hd:design-system validate --json` for design-system-specific drift checks (frontmatter validity, components-index ↔ files coverage, MDX validity, IA sync rule, no-img-in-md). High-severity findings count as L1 drift signals. After the audit, invoke `/hd:design-system tracker` to regenerate `0-welcome/Tracker.mdx` + `0-welcome/Audit.mdx` so the rendered Storybook reflects post-review state. Voice doc presence is checked via `signals.voice_docs_found` from detect.py — empty list → flag as L1 missing-voice signal (medium severity).
