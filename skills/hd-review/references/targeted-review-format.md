...
Each targeted-review response begins with a small YAML header:

```yaml
rubric: <rubric-name>                   # rubric file stem (e.g., skill-quality)
schema_version: 1                       # YAML-criteria shape; only valid value post Phase 3r
work_item: <path or URL>
composite: healthy | degraded | critical_fail
```

All adopted rubrics ship on `schema_version: 1` after Phase 3r. Custom user rubrics that don't conform produce `error: rubric-invalid` rather than fall through.
...
- severity: p1 | p2 | p3
  ...
  section_slug: <section-slug>          # kebab-case section key from the rubric YAML
  criterion_id: <criterion-id>          # kebab-case id; stable across versions
  criterion: <check-string>             # the rubric's normative check string
