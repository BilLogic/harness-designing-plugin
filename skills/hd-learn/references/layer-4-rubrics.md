...
**Standards, embedded as checks.** Evaluation design turns tacit design judgment into checks the system can actually run. Rubrics are the concrete files that make those checks inspectable:
...
The article §4d is emphatic: **evaluation is a *behavior of the system*, not a folder.** Rubrics live in three places simultaneously:

1. **Definitions** — `docs/context/design-system/` describes what "good" looks like (Layer 1 territory)
2. **Execution** — `skills/hd-review/` runs the checks (full + targeted review modes)
...
`docs/rubrics/INDEX.md` is a thin pointer file so users can FIND the evaluation story when they look. The actual rubrics execute in `hd:review` (full + targeted review modes).
...
Layer 4 is the *integration* of definitions + execution + enforcement. If one piece is missing, the evaluation layer isn't working.
...
Partially by `/hd:setup` (it creates `docs/context/design-system/` and `docs/rubrics/INDEX.md`). Fully activated by `/hd:review full` (harness-wide) or `/hd:review targeted <path>` (single work item).
