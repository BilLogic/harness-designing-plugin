...
`hd:maintain` writes lessons to `docs/knowledge/lessons/` and graduated rules to `AGENTS.md`. `docs/design-solutions/` is reserved for future distilled pattern-solutions — **not** `docs/solutions/` (compound-engineering's namespace). Keeps the two plug-ins out of each other's way.
...
`/hd:setup` scaffolds the `docs/knowledge/` tree and seeds one starter lesson. `/hd:maintain` captures ongoing lessons and proposes graduations.
