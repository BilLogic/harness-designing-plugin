...
Evaluates a Layer 2 skill (SKILL.md + its references / workflows / templates / scripts) against Anthropic's skill-authoring discipline and progressive-disclosure conventions. Apply this rubric during `hd:review` (every skill in `skills/` gets checked) or `hd:review targeted skills/<name>/SKILL.md` (focused review on one).
...
This rubric is self-referential: it's the rubric we apply to our own Layer 2 skills during `hd:review`. Grounded in Anthropic's skill-authoring guidance and the 9-section checklist Bill uses for every `hd-*` skill.
...
- As a **reviewer**, I need **the 9-section checklist as a rubric** so that **full reviews are reproducible across reviewers**.
...
- **Self-targeted review before shipping** — author walks sections 1, 2, 3, 5 on their draft. Why it matters: these four catch the most common ship-blockers.
...
**Fail example:** one skill that captures lessons, graduates them, full reviews the harness, AND targeted reviews work items.
...
`hd:review` iterates every `skills/*/SKILL.md` and runs the checks above. Severity rollup → full review report P1 / P2 / P3 buckets. Each violation cites the section number (1–9) so the fix is obvious.
...
Self-targeted review before committing a new skill: walk the 9 sections with the draft in hand. Sections 1, 2, 3, 5 catch the most common ship-blockers.
